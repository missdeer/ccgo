...
const PACKAGE_NAME = "ccgo";
const REPO_OWNER = "missdeer";
const REPO_NAME = "ccgo";
...
function getAssetName() {
  ...
  switch (platform) {
    case "darwin":
      ...
      return "ccgo_Darwin_universal.tar.gz";
    case "linux":
      if (arch === "x64") {
        return "ccgo_Linux_x86_64.tar.gz";
      } else if (arch === "arm64") {
        return "ccgo_Linux_aarch64.tar.gz";
      }
      throw new Error(
    ...
    case "win32":
      if (arch === "x64") {
        return "ccgo_Windows_x86_64.zip";
      } else if (arch === "arm64") {
        return "ccgo_Windows_aarch64.zip";
      }
      throw new Error(
...
async function downloadAndExtract(cacheDir) {
  ...
  try {
    ...
    console.error(
      "  1. Download from https://github.com/missdeer/ccgo/releases"
    );
    console.error(`  2. Place binary at: ${binaryPath}`);
    console.error("");
    console.error("Or install via cargo:");
    console.error("  cargo install ccgo");
    process.exit(1);
  } finally {
